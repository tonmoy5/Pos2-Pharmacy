...
class db
{
	...
	{
		$this->query = "
		SELECT user_name FROM user_msbs 
	...
	function fill_company()
	{
		...
		$output = '<option value="">Select Medicine Company</option>';

		foreach($result as $row)
		{
			$output .= '<option value="'.$row["medicine_manufacuter_company_id"].'">'.$row["company_name"].'</option>';
		}
		return $output;
	}
	...
	function Get_Medicine_company_code($medicine_manufacuter_company_id)
	{
		$this->query = "
		SELECT company_short_name FROM medicine_manufacuter_company_msbs 
	...
	{
		$this->query = "
		SELECT category_name FROM category_msbs 
	...
	function Get_total_no_of_medicine()
	{
		$this->query = "
		SELECT COUNT(medicine_id) AS Total FROM medicine_msbs 
		WHERE medicine_status = 'Enable' 
	...
	function Get_total_medicine_purchase()
	{
		$this->query = "
		SELECT SUM(medicine_purchase_total_cost) AS Total FROM medicine_purchase_msbs 
		WHERE medicine_purchase_status = 'Enable'
		";
	...
	function Get_total_medicine_sale()
	{
		$this->query = "
		SELECT SUM(order_total_amount) AS Total FROM order_msbs 
	...
	function Count_outstock_medicine()
	{
		$this->query = "
		SELECT COUNT(medicine_id) AS Total FROM medicine_msbs 
		WHERE medicine_status = 'Enable' 
	...
	function Get_last_fifteen_day_medicine_sale_data()
	{
	...
	function Get_medicine_name($medicine_id, $medicine_purchase_id)
	{
		$this->query = "
		SELECT * FROM medicine_msbs 
		...
		WHERE medicine_msbs.medicine_id = '$medicine_id'
		";
		...
		foreach($result as $row)
		{
			...
			$data['medicine_pack_qty'] = $row['medicine_pack_qty'] . ' ' . $this->Get_category_name($row['medicine_category']);
		...
		foreach($result as $row)
		{
			...
			$data['expiry_date'] = $row['medicine_expired_month'] . ' / ' . $row["medicine_expired_year"];
	...
	{
		$this->query = "
		SELECT store_currency FROM store_msbs 
		LIMIT 1
		";
	...
	{
		$this->query = "
		SELECT store_timezone FROM store_msbs 
		LIMIT 1
		";
	...
	function create_table()
    {
    	$this->query = "
			...
			CREATE TABLE medicine_manufacuter_company_msbs (
			  medicine_manufacuter_company_id int(11) NOT NULL,
			...
			CREATE TABLE medicine_purchase_msbs (
			  ...
			  medicine_manufacture_month varchar(30) COLLATE utf8_unicode_ci NOT NULL,
			  medicine_manufacture_year int(5) NOT NULL,
			  medicine_expired_month varchar(30) COLLATE utf8_unicode_ci NOT NULL,
			  medicine_expired_year int(5) NOT NULL,
			...
			ALTER TABLE category_msbs
  			  ADD PRIMARY KEY (category_id);
			ALTER TABLE location_rack_msbs
  			  ADD PRIMARY KEY (location_rack_id);
			ALTER TABLE medicine_manufacuter_company_msbs
			  ADD PRIMARY KEY (medicine_manufacuter_company_id);
			...
			ALTER TABLE location_rack_msbs
			  MODIFY location_rack_id int(11) NOT NULL AUTO_INCREMENT;
			ALTER TABLE medicine_manufacuter_company_msbs
			  MODIFY medicine_manufacuter_company_id int(11) NOT NULL AUTO_INCREMENT;
			ALTER TABLE medicine_msbs
  			  MODIFY medicine_id int(11) NOT NULL AUTO_INCREMENT;
			ALTER TABLE medicine_purchase_msbs
			  MODIFY medicine_purchase_id int(11) NOT NULL AUTO_INCREMENT;
			ALTER TABLE order_item_msbs
